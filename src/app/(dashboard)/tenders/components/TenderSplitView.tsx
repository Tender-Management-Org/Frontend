...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="border-t border-ink-100 px-5 py-4">
      <h3 className="text-2xs font-semibold uppercase tracking-widest text-ink-400">{title}</h3>
    ...
    <div className="min-w-0">
      ...
      <p
        className={cn(
          ...
          accent ? "font-semibold text-ink-900" : "text-ink-700"
        )}
        title={value}
      >
        {value}
  ...
  return (
    <div className="space-y-2" aria-hidden>
      {Array.from({ length: rows }).map((_, i) => (
        <div
          key={i}
          className="h-3 animate-pulse rounded-full bg-ink-100"
          style={{ width: `${92 - i * 14}%` }}
        />
      ))}
    </div>
...
function DetailPane({ tender }: { tender: TenderItem }) {
  ...
  return (
    <article ref={paneRef} className="flex max-h-[calc(100svh-13rem)] flex-col overflow-y-auto">
        ...
        <>
          ...
          <Section title="Key dates">
              ...
              {[
                ...
                .map(([label, value]) => (
                  <div key={label as string} className="flex items-baseline justify-between gap-3">
                    <dt className="shrink-0 text-2xs text-ink-400">{label}</dt>
                    <dd className="truncate text-xs tabular-nums text-ink-700">
            ...
            {work?.pre_bid_meeting_date && (
              <p className="mt-2.5 rounded-lg bg-navy-50 px-2.5 py-1.5 text-2xs text-navy-700">
                  ...
                  return (
                    <li key={`${doc.id}-${doc.s_no}-${doc.document_name}`}>
                      <a
                        ...
                        className="group flex items-center gap-2 rounded-lg border border-ink-100 px-2.5 py-1.5 transition-colors hover:border-navy-200 hover:bg-navy-50"
                      >
                        ...
                        {size && <span className="shrink-0 text-2xs tabular-nums text-ink-400">{size}</span>}
                        <Download
                          className="h-3.5 w-3.5 shrink-0 text-ink-300 transition-colors group-hover:text-navy-600"
                          aria-hidden
                        />
                      </a>
                    </li>
          ...
          {detail.tender_inviting_authority?.name && (
            <Section title="Inviting authority">
              <p className="text-sm font-medium text-ink-800">{detail.tender_inviting_authority.name}</p>
              {detail.tender_inviting_authority.address && (
                <p className="mt-1 whitespace-pre-line text-xs leading-relaxed text-ink-500">
                  {detail.tender_inviting_authority.address}
                </p>
              )}
            </Section>
      ...
      <footer className="sticky bottom-0 mt-auto flex items-center justify-between gap-3 border-t border-ink-100 bg-white/95 px-5 py-3 backdrop-blur">
        {tender.source ? (
