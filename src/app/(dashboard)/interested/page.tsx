    ...
    <section className="mx-auto w-full max-w-7xl space-y-5">
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center rounded-2xl border border-dashed border-ink-200 dark:border-ink-800 bg-surface py-20 text-center shadow-card">
          ...
          <Link
            href="/tenders"
            className="mt-6 inline-flex items-center gap-2 rounded-lg bg-navy-600 dark:bg-primary px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-navy-700 dark:hover:bg-primary/90"
          >
            Browse tenders
            <ArrowRight className="h-4 w-4" aria-hidden />
          </Link>
        ...
        <div className="space-y-3">
          {items.map((item) => {
            ...
            return (
              <article
                key={item.match_id}
                className="group rounded-2xl border border-ink-200 dark:border-ink-800 bg-surface p-5 shadow-card transition-all hover:shadow-card-hover hover:-translate-y-0.5"
              >
                {/* Top row */}
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1 space-y-1">
                    <h3 className="text-base font-semibold leading-snug text-ink-900 dark:text-ink-50">{item.title}</h3>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  <div className="rounded-lg bg-ink-50 dark:bg-ink-950 px-2.5 py-2">
                    ...
                    <p className="flex items-center gap-1 text-xs font-medium text-ink-700 dark:text-ink-200">
                  ...
                  <div className="rounded-lg bg-ink-50 dark:bg-ink-950 px-2.5 py-2">
                    ...
                    <p className="text-xs font-semibold text-ink-800 dark:text-ink-100 truncate">{formatInr(item.tender_value)}</p>
                  </div>
                  <div className="rounded-lg bg-ink-50 dark:bg-ink-950 px-2.5 py-2">
