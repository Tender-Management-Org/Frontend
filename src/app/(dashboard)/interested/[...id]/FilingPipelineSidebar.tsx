...
export function FilingPipelineSidebar({
  ...
  if (!selectedDocumentName) {
    return (
      <div className="rounded-2xl border border-dashed border-ink-200 dark:border-ink-800 bg-surface p-6 text-center shadow-card">
        <Sparkles className="mx-auto mb-3 h-8 w-8 text-ink-300 dark:text-ink-700" aria-hidden />
        <p className="text-sm font-semibold text-ink-700 dark:text-ink-200">Filing pipeline</p>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-ink-200 dark:border-ink-800 bg-surface p-5 shadow-card">
        ...
        {eligibilityCriteria.length === 0 ? (
          <p className="text-xs text-ink-400 dark:text-ink-600">No eligibility criteria extracted.</p>
        ) : (
          <ul className="max-h-64 space-y-2 overflow-y-auto pr-1">
            {eligibilityCriteria.map((item, index) => (
              <li
                key={`${item.criterion}-${index}`}
                className="rounded-xl border border-ink-100 dark:border-ink-900 bg-ink-50 dark:bg-ink-950 p-3"
              >
                <div className="flex items-start gap-2">
                  ...
                  />
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-ink-800 dark:text-ink-100">{item.criterion}</p>
                    <p className="mt-0.5 text-2xs text-ink-500 dark:text-ink-400">
                      {item.category}
                      {item.linked_document ? ` · ${item.linked_document}` : ""}
                    </p>
                    {item.notes && <p className="mt-1 text-2xs text-ink-400 dark:text-ink-600">{item.notes}</p>}
                  </div>
                </div>
              </li>
            ))}
      ...
      <div className="rounded-2xl border border-ink-200 dark:border-ink-800 bg-surface p-5 shadow-card">
        ...
        {requiredDocuments.length === 0 ? (
          <p className="text-xs text-ink-400 dark:text-ink-600">No required documents extracted.</p>
        ) : (
            ...
            {requiredDocuments.map((doc, index) => {
              ...
              return (
                <li
                  ...
                  className="rounded-xl border border-ink-100 dark:border-ink-900 p-3"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-semibold text-ink-800 dark:text-ink-100">{doc.document_name}</p>
                      <p className="mt-0.5 text-2xs text-ink-500 dark:text-ink-400">
                        {doc.type}
                  ...
                  <div className="mt-2">
                    ...
                    <label
                      htmlFor={inputId}
                      className="inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border border-ink-200 dark:border-ink-800 bg-surface px-3 text-xs font-medium text-ink-700 dark:text-ink-200 transition-colors hover:bg-ink-50 dark:hover:bg-ink-950"
                    >
                      <FileUp className="h-3.5 w-3.5" aria-hidden />
                      {uploaded ? "Replace file" : "Upload file"}
                    </label>
                    {uploaded && (
                      <p className="mt-1 truncate text-2xs text-ink-500 dark:text-ink-400">{uploaded.fileName}</p>
                    )}
                  </div>
                </li>
              );
