...
function UpgradeSuccessContent() {
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-ink-200 dark:border-ink-800 bg-surface p-8 shadow-card text-center">
        ...
        {state === "polling" && (
          <>
            ...
            <p className="mt-4 text-xs text-ink-400 dark:text-ink-600">
        ...
        {state === "success" && (
          <>
            ...
            <p className="mt-2 text-sm text-ink-500 dark:text-ink-400">
              Your <span className="font-semibold text-ink-800 dark:text-ink-100">{planName}</span> plan is now active.
              {renewDate && (
                <> Your next renewal date is <span className="font-semibold text-ink-800 dark:text-ink-100">{renewDate}</span>.</>
              )}
            </p>
            <button
              type="button"
          ...
          <>
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl border border-ink-200 dark:border-ink-800 py-2.5 text-sm font-semibold text-ink-700 dark:text-ink-200 hover:bg-ink-50 dark:hover:bg-ink-950"
              >
                Check again
              </button>
              <button
        ...
        {state === "error" && (
          <>
            ...
            {errMsg && <p className="mt-2 text-sm text-ink-500 dark:text-ink-400">{errMsg}</p>}
            <button
              type="button"
              onClick={() => router.push("/upgrade")}
